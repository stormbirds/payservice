...
import static cn.stormbirds.payservice.wx.bean.WxTransferType.*;
...
//        parameters.put("detail", order.getBody());
     ...
     *
     * @param outNo          商户转账订单号
     * @param wxTransferType 微信转账类型，.....这里没办法了只能这样写(┬＿┬)，请见谅 {@link cn.stormbirds.payservice.wx.bean.WxTransferType}
     *                       <p>
     *                       <p>
    ...
    public Map<String, Object> transferQuery(String outNo, String wxTransferType) {
        ...
        if (StringUtils.isEmpty(wxTransferType)) {
            throw new PayErrorException(new WxPayError(FAILURE, "微信转账类型 #transferQuery(String outNo, String wxTransferType) 必填，详情cn.stormbirds.payservice.wx.bean.WxTransferType"));
        }
        //如果类型为余额方式
